...
- WorkLot M:N LandLot（通过 link/relatedLandLots 表达）
- WorkLot 1:N Task

```mermaid
graph LR
  Operator[Operator] -- 1..N --> WorkLot[WorkLot]
  WorkLot -- M..N --> LandLot[LandLot]
...
| GR-06 | Task 必须关联一个 Work Lot；Work Lot 可关联一个或多个 Land Lot。 |
...
| BR-013-3 | Work Lot 可关联一个或多个 Land Lot（GR-06）。 |
...
| BR-014-2 | 可维护 relatedLandLots 关联关系。 |
...
| AC-014-2 | 角色为 Site Officer | 用户更新关联 Land Lot 并保存 | 关联关系更新 |
...
| BR ID | 规则 |
| --- | --- |
| BR-016-1 | 信息字段包含 id、operatorName、type、status、relatedLandLots、updatedBy、updatedAt。 |
