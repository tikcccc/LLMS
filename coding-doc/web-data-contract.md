...
| `contractPackage` | enum(`C1`/`C2`/`C3`) | Yes（UI 下拉） | 合約/階段維度；缺值時由來源線索推斷，預設回退 `C2` |
...
| 實體 | 欄位 | 型別 | 必填 | 說明 |
| --- | --- | --- | --- | --- |
| WorkLot | `contractPackage` | enum(`C1`/`C2`/`C3`) | Yes（UI 下拉） | 支援 GeoJSON 匯入匯出、map 渲染與 list/report 顯示；缺值時正規化回退 `C2` |
| SiteBoundary | `contractPackage` | enum(`C1`/`C2`/`C3`) | Yes（UI 下拉） | 支援 JSON/GeoJSON 匯入匯出、map 渲染與 list/report 顯示；缺值時可由 `contractNo`/`layer`/`sourceRef` 推斷，否則回退 `C2`；目前 seed data 已含 `C3`（如 `HKSTP No.5`、`HKSTP No.7`） |
...
註：store 讀取支援 legacy key（`partIdLower` / `sectionIdLower`）向後相容；新寫入一律使用 phase-scoped key，避免 `C1/C2/C3` 同 ID 互相覆蓋。Map 層級使用 `activeContract`（workspace）一次只顯示單一 contract。
