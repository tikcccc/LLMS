...
- 現況檔案經 gzip 可降到原始大小約 22.2%，代表「網路傳輸」可壓縮，但「後端序列化 + 前端解析」成本仍存在。
