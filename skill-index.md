...
| `dxf-geojson-ops` | 處理 DXF -> GeoJSON 轉檔流程、參數調整、輸出檢查與前端路徑同步（含 C1 的 the part of sites：`dxf_drawings/C1_DXF`） | 問「幫我把 C1_DXF 的 the part of sites 轉成前端可用資料」、「轉檔品質要怎麼驗證」 |
| `function-update` | 針對新功能或功能變更做端到端更新：實作、文件同步、驗證 | 問「新增一個功能」、「完善現有功能並驗證可用性」 |
| `code-structure-split` | 當程式文件過於臃腫時，進行結構性拆分、合理命名與落位，降低後續改動風險 | 問「這個檔案太肥不好維護，先幫我拆模組但不要改功能」、「重構成可持續迭代的結構」 |
...
3. DXF 轉檔與 GIS GeoJSON 驗證（含 `dxf_drawings/C1_DXF` 的 the part of sites）：`@dxf-geojson-ops`
4. 新功能新增/功能改造（含驗證與文檔同步）：`@function-update`
5. 當單一檔案過胖、責任混雜、後續 agent 難以修改：`@code-structure-split`
6. PM 提出新需求需先做業務對齊：`@pm-business-function-consistency`
7. 業務背景/術語/需求問答與知識更新：`@business-context-qa-sync`
8. 偵測 reference-doc 更新並自動路由文檔同步：`@business-doc-index-update`
9. 先做大檔拆分再改功能：`@code-structure-split` -> `@function-update` -> `@coding-doc`
10. 同時有前端改動 + 文檔同步：`@pm-business-function-consistency` -> `@function-update`（或 `@frontend`）-> `@coding-doc`
