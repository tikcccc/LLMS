...
function debounce(fn, time2) {
  ...
  return function debounced(...args) {
    reset();
    state.args = args;
    state.timer = window.setTimeout(run, time2);
    return state.result;
  };
}
var VTYPE_ELEMENT = 1;
...
var isLeaf = (c3) => typeof c3 === "string" || typeof c3 === "number";
var isElement = (c3) => (c3 == null ? void 0 : c3.vtype) === VTYPE_ELEMENT;
var isRenderFunction = (c3) => (c3 == null ? void 0 : c3.vtype) === VTYPE_FUNCTION;
...
function blurh(blur3, T2, S, w, h2) {
  for (let y3 = 0, n = w * h2; y3 < n; ) {
    blur3(T2, S, y3, y3 += w, 1);
  }
}
function blurv(blur3, T2, S, w, h2) {
  for (let x3 = 0, n = w * h2; x3 < w; ++x3) {
...
function hslConvert(o) {
  ...
  o = o.rgb();
  var r = o.r / 255, g = o.g / 255, b = o.b / 255, min4 = Math.min(r, g, b), max5 = Math.max(r, g, b), h2 = NaN, s2 = max5 - min4, l = (max5 + min4) / 2;
  if (s2) {
    if (r === max5) h2 = (g - b) / s2 + (g < b) * 6;
    else if (g === max5) h2 = (b - r) / s2 + 2;
    else h2 = (r - g) / s2 + 4;
...
function cubehelixConvert(o) {
  ...
  if (!(o instanceof Rgb)) o = rgbConvert(o);
  var r = o.r / 255, g = o.g / 255, b = o.b / 255, l = (BC_DA * b + ED * r - EB * g) / (BC_DA + ED - EB), bl = b - l, k2 = (E * (g - l) - C * bl) / D, s2 = Math.sqrt(k2 * k2 + bl * bl) / (E * l * (1 - l)), h2 = s2 ? Math.atan2(k2, bl) * degrees - 120 : NaN;
...
function hsl2(hue2) {
  return function(start2, end) {
    var h2 = hue2((start2 = hsl(start2)).h, (end = hsl(end)).h), s2 = nogamma(start2.s, end.s), l = nogamma(start2.l, end.l), opacity = nogamma(start2.opacity, end.opacity);
    return function(t) {
      start2.h = h2(t);
      start2.s = s2(t);
      start2.l = l(t);
...
function hcl2(hue2) {
  return function(start2, end) {
    var h2 = hue2((start2 = hcl(start2)).h, (end = hcl(end)).h), c3 = nogamma(start2.c, end.c), l = nogamma(start2.l, end.l), opacity = nogamma(start2.opacity, end.opacity);
    return function(t) {
      start2.h = h2(t);
      start2.c = c3(t);
      start2.l = l(t);
...
function cubehelix2(hue2) {
  return function cubehelixGamma(y3) {
    ...
    function cubehelix3(start2, end) {
      var h2 = hue2((start2 = cubehelix(start2)).h, (end = cubehelix(end)).h), s2 = nogamma(start2.s, end.s), l = nogamma(start2.l, end.l), opacity = nogamma(start2.opacity, end.opacity);
      return function(t) {
        start2.h = h2(t);
        start2.s = s2(t);
...
var Path = class {
  ...
  rect(x3, y3, w, h2) {
    this._append`M${this._x0 = this._x1 = +x3},${this._y0 = this._y1 = +y3}h${w = +w}v${+h2}h${-w}Z`;
  }
  toString() {
    return this._;
  }
...
function St(e) {
  var t = this, i, n = [t], s2, o, h2;
  do
    for (i = n.reverse(), n = []; t = i.pop(); )
      if (e(t), s2 = t.children, s2) for (o = 0, h2 = s2.length; o < h2; ++o)
        n.push(s2[o]);
  while (n.length);
  return this;
}
...
function Rt(e) {
  for (var t = this, i = [t], n = [], s2, o, h2; t = i.pop(); )
    if (n.push(t), s2 = t.children, s2) for (o = 0, h2 = s2.length; o < h2; ++o)
      i.push(s2[o]);
  for (; t = n.pop(); )
    e(t);
  return this;
...
function j(e, t) {
  var i = new _(e), n = +e.value && (i.value = e.value), s2, o = [i], h2, a2, c3, l;
  for (t == null && (t = Ft); s2 = o.pop(); )
    if (n && (s2.value = +s2.data.value), (a2 = t(s2.data)) && (l = a2.length))
      for (s2.children = new Array(l), c3 = l - 1; c3 >= 0; --c3)
        o.push(h2 = s2.children[c3] = new _(a2[c3])), h2.parent = s2, h2.depth = s2.depth + 1;
...
var et = (e, t = 0) => (e.y = t, (e.children || []).reduce((i, n) => {
  const [s2, o] = i;
  et(n, e.y + e.ySize);
  const h2 = (s2 === 0 ? n.lExt : n.rExt).bottom;
  s2 !== 0 && Kt(e, s2, o);
  const a2 = Ut(h2, s2, o);
  return [s2 + 1, a2];
}, [0, null]), It(e), Qt(e), e);
...
var It = (e) => {
  (e.children || []).reduce((t, i) => {
    const [n, s2] = t, o = n + i.shift, h2 = s2 + o + i.change;
    return i.relX += h2, [o, h2];
  }, [0, 0]);
};
var Kt = (e, t, i) => {
  const n = e.children[t - 1], s2 = e.children[t];
  let o = n, h2 = n.relX, a2 = s2, c3 = s2.relX, l = true;
  for (; o && a2; ) {
    o.bottom > i.lowY && (i = i.next);
    const d = h2 + o.prelim - (c3 + a2.prelim) + o.xSize / 2 + a2.xSize / 2 + o.spacing(a2);
    (d > 0 || d < 0 && l) && (c3 += d, Gt(s2, d), Vt(e, t, i.index, d)), l = false;
    const p = o.bottom, m = a2.bottom;
    p <= m && (o = Zt(o), o && (h2 += o.relX)), p >= m && (a2 = Yt(a2), a2 && (c3 += a2.relX));
  }
  !o && a2 ? qt(e, t, a2, c3) : o && !a2 && Jt(e, t, o, h2);
};
var Gt = (e, t) => {
...
var Vt = (e, t, i, n) => {
  ...
  if (o > 1) {
    const h2 = n / o;
    e.children[i + 1].shift += h2, s2.shift -= h2, s2.change -= n - h2;
  }
};
var Yt = (e) => e.hasChildren ? e.firstChild : e.lThr;
var Zt = (e) => e.hasChildren ? e.lastChild : e.rThr;
var qt = (e, t, i, n) => {
  const s2 = e.firstChild, o = s2.lExt, h2 = e.children[t];
  o.lThr = i;
  const a2 = n - i.relX - s2.lExtRelX;
  o.relX += a2, o.prelim -= a2, s2.lExt = h2.lExt, s2.lExtRelX = h2.lExtRelX;
};
var Jt = (e, t, i, n) => {
  const s2 = e.children[t], o = s2.rExt, h2 = e.children[t - 1];
  o.rThr = i;
  const a2 = n - i.relX - s2.rExtRelX;
  o.relX += a2, o.prelim -= a2, s2.rExt = h2.rExt, s2.rExtRelX = h2.rExtRelX;
};
var Qt = (e) => {
  if (e.hasChildren) {
...
`);
  ...
  _initializeData(t) {
    ...
    return walkTree(t, (a2, c3, l) => {
      var p, m, u4, x3;
      h2 += 1, a2.children = (p = a2.children) == null ? void 0 : p.map((y3) => ({ ...y3 })), i += 1, a2.state = {
        ...a2.state,
        depth: h2,
        id: i,
        rect: {
      ...
      d ? o += 1 : (o || s2 >= 0 && a2.state.depth >= s2) && (a2.payload = { ...a2.payload, fold: 1 }), c3(), d && (o -= 1), h2 -= 1;
    }), t;
  }
  _relayout() {
    ...
    }).spacing((c3, l) => (c3.parent === l.parent ? s2 : s2 * 2) + t(c3.data)), h2 = o.hierarchy(this.state.data);
    o(h2);
    const a2 = h2.descendants();
    a2.forEach((c3) => {
      const l = c3.data;
      l.state.rect = {
        x: c3.y,
  ...
  async renderData(t) {
    const { paddingX: i, autoFit: n, color: s2, maxWidth: o, lineWidth: h2 } = this.options, a2 = this.state.data;
    if (!a2) return;
    const c3 = {}, l = {}, d = [];
    walkTree(a2, (r, g, f) => {
      var E2;
    ...
    this.transition(ct).attr("x1", (r) => r.state.rect.width).attr("stroke-width", 0), N.attr("x1", (r) => r.state.rect.width).attr("x2", (r) => r.state.rect.width), B3.attr("y1", (r) => r.state.rect.height + h2(r) / 2).attr("y2", (r) => r.state.rect.height + h2(r) / 2), this.transition(B3).attr("x1", -1).attr("x2", (r) => r.state.rect.width + 2).attr("stroke", (r) => s2(r)).attr("stroke-width", h2);
    const ht = S.selectAll(
      v2("circle")
    );
    this.transition(ht).attr("r", 0).attr("stroke-width", 0), D3.attr("cx", (r) => r.state.rect.width).attr("cy", (r) => r.state.rect.height + h2(r) / 2), this.transition(D3).attr("r", 6).attr("stroke-width", "1.5"), this.transition(I).style("opacity", 0), K2.attr("width", (r) => Math.max(0, r.state.rect.width - i * 2)).attr("height", (r) => r.state.rect.height), this.transition(K2).style("opacity", 1), this.transition(at).attr("d", (r) => {
      const g = y3(r.target), f = [
        g.x + g.width,
        g.y + g.height + h2(r.target) / 2
      ];
      return H({ source: f, target: f });
    }).attr("stroke-width", 0).remove(), this.transition(ot).attr("stroke", (r) => s2(r.target)).attr("stroke-width", (r) => h2(r.target)).attr("d", (r) => {
  ...
  async fit(t = this.options.maxInitialScale) {
    const i = this.svg.node(), { width: n, height: s2 } = i.getBoundingClientRect(), { fitRatio: o } = this.options, { x1: h2, y1: a2, x2: c3, y2: l } = this.state.rect, d = c3 - h2, p = l - a2, m = Math.min(
      n / d * o,
      s2 / p * o,
      t
  ...
  async ensureVisible(t, i) {
    ...
    if (!n) return;
    const s2 = this.svg.node(), o = s2.getBoundingClientRect(), h2 = transform(s2), [a2, c3] = [
      n.state.rect.x,
      ...
      bottom: 0,
      ...i
    }, m = [p.left - a2, o.width - p.right - c3], u4 = [p.top - l, o.height - p.bottom - d], x3 = m[0] * m[1] > 0 ? Y2(m, Math.abs) / h2.k : 0, y3 = u4[0] * u4[1] > 0 ? Y2(u4, Math.abs) / h2.k : 0;
    if (x3 || y3) {
      const b = h2.translate(x3, y3);
  ...
  async centerNode(t, i) {
    ...
    if (!n) return;
    const s2 = this.svg.node(), o = s2.getBoundingClientRect(), h2 = transform(s2), a2 = (n.state.rect.x + n.state.rect.width / 2) * h2.k + h2.x, c3 = (n.state.rect.y + n.state.rect.height / 2) * h2.k + h2.y, l = {
      left: 0,
      right: 0,
      top: 0,
      bottom: 0,
      ...i
    }, d = (l.left + o.width - l.right) / 2, p = (l.top + o.height - l.bottom) / 2, m = (d - a2) / h2.k, u4 = (p - c3) / h2.k;
    if (m || u4) {
      const y3 = h2.translate(m, u4);
   ...
   */
  async rescale(t) {
    const i = this.svg.node(), { width: n, height: s2 } = i.getBoundingClientRect(), o = n / 2, h2 = s2 / 2, a2 = transform(i), c3 = a2.translate(
      (o - a2.x) * (1 - t) / a2.k,
      (h2 - a2.y) * (1 - t) / a2.k
    ).scale(t);
